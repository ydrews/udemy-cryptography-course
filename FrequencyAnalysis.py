import operator
import sys

cipher = """lrvmnir bpr sumvbwvr jx bpr lmiwv yjeryrkbi jx qmbm wi
bpr xjvni mkd ymibrut jx irhx wi bpr riirkvr jx
ymbinlmtmipw utn qmumbr dj w ipmhh but bj rhnvwdmbr bpr
yjeryrkbi jx bpr qmbm mvvjudwko bj yt wkbrusurbmbwjk
lmird jk xjubt trmui jx ibndt
  wb wi kjb mk rmit bmiq bj rashmwk rmvp yjeryrkb mkd wbi
iwokwxwvmkvr mkd ijyr ynib urymwk nkrashmwkrd bj ower m
vjyshrbr rashmkmbwjk jkr cjnhd pmer bj lr fnmhwxwrd mkd
wkiswurd bj invp mk rabrkb bpmb pr vjnhd urmvp bpr ibmbr
jx rkhwopbrkrd ywkd vmsmlhr jx urvjokwgwko ijnkdhrii
ijnkd mkd ipmsrhrii ipmsr w dj kjb drry ytirhx bpr xwkmh
mnbpjuwbt lnb yt rasruwrkvr cwbp qmbm pmi hrxb kj djnlb
bpmb bpr xjhhjcwko wi bpr sujsru msshwvmbwjk mkd
wkbrusurbmbwjk w jxxru yt bprjuwri wk bpr pjsr bpmb bpr
riirkvr jx jqwkmcmk qmumbr cwhh urymwk wkbmvb"""


class Attack:
    def __init__(self):
        self.alphabet = "abcdefghijklmnopqrstuvwxyz"
        self.p_chars_left = "abcdefghijklmnopqrstuvwxyz"
        self.c_chars_left = "abcdefghijklmnopqrstuvwxyz"
        self.freq = {}
        self.key = {}
        self.mapping = {}
        self.freq_eng = {
            'a': 0.0817, 'b': 0.0150, 'c': 0.0278, 'd': 0.0425, 'e': 0.1270, 'f': 0.0223,
            'g': 0.0202, 'h': 0.0609, 'i': 0.0697, 'j': 0.0015, 'k': 0.0077, 'l': 0.0403,
            'm': 0.0241, 'n': 0.0675, 'o': 0.0751, 'p': 0.0193, 'q': 0.0010, 'r': 0.0599,
            's': 0.0633, 't': 0.0906, 'u': 0.0276, 'v': 0.0098, 'w': 0.0236, 'x': 0.0015,
            'y': 0.0197, 'z': 0.0007
        }

    def calculate_freq(self, cipher):
        for c in self.alphabet:
            self.freq[c] = 0

        count = 0
        for c in cipher:
            if c in self.freq:
                self.freq[c] += 1
                count += 1

        for c in self.freq:
            self.freq[c] = round(self.freq[c] / count, 4)

    def print_freq(self):
        new_line_count = 0
        for c in self.freq:
            print(c, ':', self.freq[c], ' ', end='')
            if new_line_count % 3 == 2:
                print()
            new_line_count += 1
        # for c in self.freq:
        #     print(c, ':', self.freq[c])

    def calculate_matches(self):
        for c_char in self.alphabet:
            map = {}
            for p_char in self.alphabet:
                map[p_char] = round(
                    abs(self.freq[c_char] - self.freq_eng[p_char]), 4)
            self.mapping[c_char] = sorted(
                map.items(), key=operator.itemgetter(1))

    def set_key_mapping(self, c_char, p_char):
        if c_char not in self.c_chars_left or p_char not in self.p_chars_left:
            print("ERROR: key mapping error", c_char, p_char)
            sys.exit(-1)
        self.key[c_char] = p_char
        self.p_chars_left = self.p_chars_left.replace(p_char, '')
        self.c_chars_left = self.c_chars_left.replace(c_char, '')

    def guess_key(self):
        for c_char in self.c_chars_left:
            for p_char, diff in self.mapping[c_char]:
                if p_char in self.p_chars_left:
                    self.key[c_char] = p_char
                    self.p_chars_left = self.p_chars_left.replace(p_char, '')
                    break

    def get_key(self):
        return self.key


def decrypt(key, cipher):
    message = ""
    for c in cipher:
        if c in key:
            message += key[c]
        else:
            message += c
    return message


attack = Attack()
attack.calculate_freq(cipher)
attack.print_freq()
attack.calculate_matches()

attack.set_key_mapping('a', 'x')
attack.set_key_mapping('b', 't')
attack.set_key_mapping('c', 'w')
attack.set_key_mapping('d', 'd')
attack.set_key_mapping('e', 'v')
attack.set_key_mapping('f', 'q')
attack.set_key_mapping('g', 'z')
# attack.set_key_mapping('h', '')
# attack.set_key_mapping('i', '')
attack.set_key_mapping('j', 'o')
# attack.set_key_mapping('k', '')
attack.set_key_mapping('l', 'b')
attack.set_key_mapping('m', 'a')
# attack.set_key_mapping('n', '')
attack.set_key_mapping('o', 'g')
attack.set_key_mapping('p', 'h')
attack.set_key_mapping('q', 'k')
attack.set_key_mapping('r', 'e')
attack.set_key_mapping('s', 'p')
attack.set_key_mapping('t', 'y')
attack.set_key_mapping('u', 'r')
attack.set_key_mapping('v', 'c')
attack.set_key_mapping('w', 'i')
attack.set_key_mapping('x', 'f')
# attack.set_key_mapping('y', '')
# attack.set_key_mapping('z', '')

attack.guess_key()
key = attack.get_key()

print()
print(key)
message = decrypt(key, cipher)
message_lines = message.splitlines()
cipher_lines = cipher.splitlines()

for i in range(len(message_lines)):
  print("C: ", cipher_lines[i])
  print("P: ", message_lines[i])
  print()

print(message)
